...
\glqq JAP\grqq\ ist ein Mix\-kaskaden-basierter Dienst zur Anonymisierung von
...
verschlüsselte Datenpakete emp\-fängt, zwischenpuffert, umkodiert (durch
Entschlüsselung eine \glqq Ver\-schlüs\-selungs\-schale\grqq\ entfernt) und
...
Mix der Kaskade ver\-trau\-ens\-wür\-dig ist. Dies gilt selbst unter der Annahme, dass
ein Angreifer alle anderen Mixe kontrolliert und sämtliche
Netzwerkverbindungen überwachen bzw. aktiv manipulieren kann (d.\thinspace h.
Pakete löscht, verändert, hinzufügt etc.)

Betrachtet man eine Kaskade als \glqq Black Box\grqq, so ist das Ziel, die Zuordnung von
...
Beispielsweise \glqq sieht\grqq\ ein Angreifer, wer welches \glqq Bitmuster\grqq\ 
an den ersten Mix gesendet hat. 

Jedem Mixbetreiber entstehen Kosten, die hauptsäch\-lich durch das
Datenvolumen der zu bearbeitenden Mixpakete verursacht werden \cite{leipzig}.
Eine faire Mög\-lichkeit zur Deckung der anfallenden Kosten besteht darin, diese
auf die Nutzer umzulegen.

Bisherige Ansätze~\cite{bezahl1, bezahl2,
bezahl3} nutzen zur Bezahlung der Mixpakete anonyme digitale
...
\section{Architektur des Bezahlsystems}
\label{struktur}
...
einzeln bezahlt, sondern eine Kaskaden-zentrale \emph{Abrechnungsinstanz} (AI). 
...
\emph{Guthabenkonto}, von dem die Kosten abgebucht werden. Dieses muss er
...
aufzuladendes Konto benötigt, verwaltet eine Anonymisierungsdienst-zentrale \emph{Bezahlinstanz} (BI)
...
Bezahlinstanz, Abrechnungsinstanzen und lokale Proxies der Nutzer (Abbildung~\ref{architektur}).
...
\end{verbatim}
Beim Beginn einer Sitzung kann der JAP mittels HTTP POST Request an die URL "/authenticate" eine Challenge anfordern. Dazu sendet er
im HTTP Body sein Kontozertifikat mit.

Als Response erwartet die BI eine Signatur über das Element <DontPanic> (inklusive der Tags) vom JAP. Die Signatur leistet der JAP
...
\subsection{JAP-Bezahlinstanz-Protokoll}
\label{jap-bi}

\subsubsection{Kontoanmeldung} 
\label{kontoanmeld}
...
Schlüssel wird als Nutzerpseudonym $P$ an ein Konto bei der BI gebunden (Abbildung~\ref{kontoanmeldung}).
...
Der JAP über\-mit\-telt zunächst den generierten öf\-fent\-lichen Schlüssel. 
Die BI erzeugt eine für sie eindeutige
Kontonummer \textit{KN}\ und legt unter dieser ein Konto für den Nutzer an.
Der öffentliche Schlüssel und die Kontonummer werden zusammen von der BI
signiert und das resultierende Kontozertifikat
$$Z=(BI,P,\textit{KN},t_{\text{creation}},S_{\text{BI}})$$ an den JAP
zurückgesendet.  $BI$ ist dabei ein eindeutiger Identifikator der Bezahlinstanz, $S_{\text{BI}}$ ist die digitale Signatur der BI über
$BI,P,\textit{KN}$ und $t_{\text{creation}}$. Der Zeitpunkt $t_{\text{creation}}$
...
\paragraph{Implementierungsanmerkungen}
Zu Kontoanmeldung ruft der JAP bei der BI die URL "/register" mittels POST Request auf. Im Body wird mittels einer XML-Struktur der
...
\end{verbatim}
 ...
 Der JAP antwortet mittels POST Request an die URL "/response". Im HTTP Request-Body befindet sich dabei die XML <Response> Struktur.
...
In der Datenbanktabelle ACCOUNTS wird ein neuer Eintrag angelegt mit der erzeugten Kontonummer, 
dem Zeitstempel der Erzeugung, sowie dem zugehörigen öffentlichen Schlüssel. Die Werte DEPOSIT und SPENT werden auf Null gesetzt.
...
\subsubsection{Kontoaufladung}

Zur Dienstnutzung muss das angemeldete Pseudonymkonto aufgeladen werden.  Der
JAP erfragt dafür bei der BI eine Überweisungsnummer \textit{ÜN} als einmal gül\-tiges
Transaktionspseudonym. Die Überweisungsnummer wird durch die BI generiert,
dort vermerkt und dem JAP zusammen mit der Pseudonymkontonummer und dem aktuellen $G_{\max}$ von ihr
...
Die BI erzeugt ausserdem einen Eintrag in der Datenbanktabelle TRANSFERS. Der Wert USED wird dabei auf 'false' gesetzt. 
Die BI überträgt nun das oben erstellte Transaktionszertifikat an den Nutzer zusammen mit einer URL, wo genaue Erläuterungen zur Bezahlung und zur Benutzung von elektronischen Bezahlverfahren hinterlegt sind....  

\subsubsection{Kontostandsabfrage}
\label{kontoabfrag}
...
\ref{ai-bi} aus\-führ\-lich erläutert.
...
\paragraph{Sicherheitsbetrachtungen}
...
\subsection{JAP-Abrechnungsinstanz-Protokoll}
\label{ai-jap}
...
%\paragraph{Micropayment} \todo{Kürzen} 
...
\subsection{Abrechnungsinstanz-Bezahlinstanz-Protokoll}
\label{ai-bi}

Die Abrechnungsinstanzen kontaktieren in kurzfristigen Abständen die
Bezahl\-instanz zur Kostenbestätigungsabrechnung. In längerfristigen
Intervallen, \zb einmal im Monat, wird die BI von den AIs zur Geldabrechnung
kontaktiert.

\paragraph{Abrechnung der Kostenbestätigungen}
...
\paragraph{Kontostandsaktualisierung}
...
\paragraph{Geldauszahlung an die Abrechnungsinstanzen}
...
\end{verbatim}
\subsection{Transaktionsnummern}
\begin{verbatim}
Name: TRANSFERS
Felder: 
	ACCOUNTNUMBER    (64 Bit Integer; BIGINTEGER) - Kontonummer
...
\end{verbatim}
\subsection{BI Kostenbestätigungen pro Konto und Kaskade}
\begin{verbatim}
...
\end{verbatim}		
\subsection{AI Kostenbestätigungen}
\begin{verbatim}
Name: COSTCONFIRMATIONS
Felder: 
	ACCOUNTNUMBER    (64 Bit Integer; BIGINTEGER) - Kontonummer
